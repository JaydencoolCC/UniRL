...
UniGRPO (Liu et al.) replaces FlowGRPO's standard latent-KL penalty with an
**unweighted MSE on the predicted velocity field** (paper Eq. 8)::

    L_MSE(theta) = || v_theta(x_t, t, y) - v_ref(x_t, t, y) ||^2

evaluated at the SDE-trained timesteps, where ``v_ref`` is the frozen pre-trained
base reference. Under LoRA that is the policy with adapters disabled; under full
fine-tuning (no adapters to disable) it is a frozen snapshot of the base weights
swapped into the trainable params only for the v_ref forward. This pulls the
RL-tuned vector field uniformly back toward the base across all noise levels, which
the paper finds mitigates reward hacking better than the timestep-weighted latent KL.

This subclasses :class:`FlowGRPO`: the clipped policy-gradient surrogate is
unchanged (inherited), and the MSE term is added with its own backward (the
gradients accumulate into the same optimizer step). RatioNorm (GRPO-Guard) is
intentionally NOT included in v1 — it is a separate follow-up pending the exact
GRPO-Guard normalization constant.

Compute note (v1, correctness-first): the MSE runs two extra velocity forwards
per SDE step (``v_theta`` with grad, ``v_ref`` with adapters off), independent of
the inherited GRPO log-prob replay. Fusing ``v_theta`` with the replay forward is
a follow-up optimization; it would require the diffusion stage to surface the
per-step velocity it already computes.
"""
...
class BagelFlowUniGRPO(FlowGRPO):
    ...
    def __init__(
        ...
        *,
        ...
        # GRPO-Guard (arXiv:2510.22319): ratio_norm normalizes the flow importance
        # ratio per SDE step so PPO clipping actually engages (the ratio is otherwise
        # left-shifted, mean<1). grad_reweight (×1/|dt|) is the optional 2nd component
        # ("modest gains"; off by default — UniGRPO specifies only RatioNorm).
    ...
    # ------------------------------------------------------------------
    # GRPO-Guard RatioNorm (arXiv:2510.22319) surrogate
    # ------------------------------------------------------------------
